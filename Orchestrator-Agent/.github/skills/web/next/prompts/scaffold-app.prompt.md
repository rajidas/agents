...
- Inspect the package manager, `package.json`, scripts, configuration, and
	dependencies before selecting test tooling. Reuse an existing compatible
	runner; otherwise configure Jest for unit/integration tests and use Playwright
	for browser journeys. Add focused smoke and unit/integration coverage for the
	acceptance criteria, relevant states, and highest-risk regression paths.
